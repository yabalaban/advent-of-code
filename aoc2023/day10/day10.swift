...
func enclosingTiles(_ pipes: String) -> Int {
    let rlen = pipes.firstIndex(of: "\n")!.utf16Offset(in: pipes)
    let pipes = pipes.replacingOccurrences(of: "\n", with: "")
    let sidx = pipes.firstIndex(of: "S")!
    let spos = sidx.utf16Offset(in: pipes)
...
func problem_10a() async throws -> Int {
    guard let url = Bundle.main.url(forResource: "day10", withExtension: "txt") else {
        fatalError()
    }
...
func problem_10b() async throws -> Int {
    guard let url = Bundle.main.url(forResource: "day10", withExtension: "txt") else {
        fatalError()
    }
    
    { // assertions
        assert(enclosingTiles("""
.....
.S-7.
.|.|.
.L-J.
