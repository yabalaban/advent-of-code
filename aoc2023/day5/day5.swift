...
func resolveAlmanac(_ almanac: String) -> [Int] {
    var seeds: [Int] = []
    var maps: [Map] = []
    
    let lines = almanac.split(separator: "\n\n")
...
func resolveAlmanacRanged(_ almanac: String) -> Int {
    var seeds: [Int] = []
    var maps: [Map] = []
...
func problem_5a() async throws -> Int {
    guard let url = Bundle.main.url(forResource: "day5", withExtension: "txt") else {
        fatalError()
    }
    
    { // assertions
        assert(resolveAlmanac("""
seeds: 79 14 55 13
...
""") == [82, 43, 86, 35])
    ...
    let almanac = try String(contentsOfFile: url.path(), encoding: .utf8)
    return resolveAlmanac(almanac).min()!
}

func problem_5b() async throws -> Int {
    guard let url = Bundle.main.url(forResource: "day5", withExtension: "txt") else {
        fatalError()
    }
    
    { // assertions
        assert(resolveAlmanacRanged("""
seeds: 79 14 55 13
...
""") == 46)
    ...
    let almanac = try String(contentsOfFile: url.path(), encoding: .utf8)
    return resolveAlmanacRanged(almanac)
}
